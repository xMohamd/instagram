    ...
    <div class="flex justify-center">
        <div class="w-full md:w-11/12 lg:w-10/12 xl:w-8/12">
            <div class="bg-white shadow-gradient rounded-lg overflow-hidden animate-fade-in">
                <div class="p-5 bg-gradient-to-r from-pink-500 to-purple-500 text-white flex justify-between items-center">
                    <h2 class="text-2xl font-bold">Admin Dashboard</h2>
                    <a href="{{ route('admin.trash') }}" class="btn-gradient py-2 px-4 rounded transition ease-in-out duration-150">
                        Trash
                    </a>
                </div>
                        ...
                        @foreach ($users as $user)
                            <tr class="hover:bg-gray-100">
                                <td class="px-6 py-4 flex items-center">
                                    <img class="h-10 w-10 rounded-full mr-4" src="{{ $user->avatar ?? 'default-image-url' }}" alt="{{ $user->name }}'s profile picture">
                                    {{ $user->name }}
                                </td>
                                ...
                                <td class="px-6 py-4 flex space-x-4">
                                    <a href="{{ route('admin.edit', $user->id) }}" class="btn-edit py-2 px-4 rounded transition ease-in-out duration-150">Edit</a>
                                    <form action="{{ route('admin.destroy', $user->id) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn-delete py-2 px-4 rounded transition ease-in-out duration-150">Delete</button>
                                    </form>
                                    @if($user->deleted_at !== null)
                                        <a href="{{ route('admin.restore', $user->id) }}" class="text-green-500 hover:text-green-700">Restore</a>
                                    @endif
                                </td>
                            </tr>
                        @endforeach
